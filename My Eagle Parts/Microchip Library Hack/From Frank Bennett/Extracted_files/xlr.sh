...
X
SHAR_EOF
  (set 20 08 04 18 17 38 29 'x2lib.c'; eval "$shar_touch") &&
  chmod 0644 'x2lib.c'
if test $? -ne 0
...
if test -f 'XLR_Format.html' && test "$first_param" != -c; then
  ${echo} 'x -SKIPPING XLR_Format.html (file already exists)'
else
...
SHAR_EOF
  (set 20 08 04 14 18 14 11 'XLR_Format.html'; eval "$shar_touch") &&
  chmod 0755 'XLR_Format.html'
if test $? -ne 0
then ${echo} 'restore of XLR_Format.html failed'
fi
  if ${md5check}
  then (
       ${MD5SUM} -c >/dev/null 2>&1 || ${echo} 'XLR_Format.html: MD5 check failed'
       ) << SHAR_EOF
96f9cef3a180d3a17ec578acb4584f04  XLR_Format.html
SHAR_EOF
  else
test `LC_ALL=C wc -c < 'XLR_Format.html'` -ne 127919 && \
  ${echo} 'restoration warning:  size of XLR_Format.html is not 127919'
  fi
fi
# ============= XLR_Format.txt ==============
...
SHAR_EOF
  (set 20 08 04 19 09 41 45 'XLR_Format.txt'; eval "$shar_touch") &&
...
*/
SHAR_EOF
  (set 20 08 04 15 12 46 14 'xlr.h'; eval "$shar_touch") &&
  chmod 0644 'xlr.h'
if test $? -ne 0
...
SHAR_EOF
  (set 20 08 04 18 17 43 42 'xlr.y'; eval "$shar_touch") &&
  chmod 0644 'xlr.y'
if test $? -ne 0
